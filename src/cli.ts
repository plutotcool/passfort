...
if (cmd === 'matcher') {
  ...
  if (block) {
    console.log(
      '\n# Block entire site (maintenance mode). Paste into middleware.ts\n'
    );
    console.log(MATCHER_BLOCK_SNIPPET);
    console.log('');
  } else {
    console.log('\n# Protect entire site. Paste into middleware.ts\n');
    console.log(MATCHER_SNIPPET);
    console.log(
...
if (cmd === 'init') {
  if (args.includes('--help') || args.includes('-h')) {
    console.log(`
passfort init - Create middleware.ts and add passfort

  npx passfort init              Protect entire site
...
`);
  ...
  const result = runInit({ startDir: cwd, block, paths });
...
Commands:
  init [options]        Create middleware.ts (if none) and add passfort
  hash <password>       Generate PBKDF2 hash for production use
  matcher [--block]     Print middleware snippet (protect or block entire site)
...
Quick start:
  ...
  2. npx passfort init              # create middleware (entire site protected)
     or npx passfort init --paths=/admin,/dashboard
  3. Set env: PASSFORT_PASSWORD, PASSFORT_SECRET

Manual: npx passfort matcher        # paste output into middleware.ts
